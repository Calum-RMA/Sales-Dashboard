...
export default function CallAssessments({ refreshKey = 0 }) {
  ...
  const repAvg = new Map(repRows.map((r) => [r.rep, r.av]));
            ...
            <thead><tr>
              <th />{repGrid.names.map((n) => (
                <th key={n} style={{ ...th, textAlign: "center", whiteSpace: "nowrap" }}>
                  {n.split(" ")[0]}<br /><span style={{ fontSize: 10, fontWeight: 500 }}>{repGrid.calls.get(n).size} calls</span><br /><span style={{ fontSize: 11, fontWeight: 800, color: "#91c7e8" }}>{pctText(repAvg.get(n))} avg</span>
                </th>
              ))}
            </tr></thead>
            <tbody>
